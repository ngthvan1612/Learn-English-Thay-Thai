...
export default function CreateMultipleChoiceModal(props) {
  ...
  function updateListSelection() {
    setListAnswerSelections(responseForm.getFieldValue('responses').map((resp, index) => {
  ...
  return (
    ...
    >
      <Button onClick={() => localStorage.setItem('Q1', JSON.stringify(multipleChoiceData))}>Test lưu</Button>
      <Form  
      >
        <Form.Item label='Nội dung câu hỏi'>
          <Input.TextArea onChange={(e) => updateQuestionTitle(e.target.value)}/>
        </Form.Item>
      </Form>
      <Form form={responseForm} {...formItemLayout}
        style={{
          width: '100%',
          marginBottom: 0
        }}>
      ...
      <Form>
        <Form.Item label='Đán án'>
          <Select options={listAnswerSelections} onChange={(value) => updateAnswer(value)}>
          </Select>
        </Form.Item>
        <Form.Item label='Lời giải'>
          <Input.TextArea onChange={(e) => updateSolution(e.target.value)}/>
        </Form.Item>
      </Form>
    </Modal>
  )
